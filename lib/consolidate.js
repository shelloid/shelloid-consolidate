/*!
 * consolidate
 * Copyright(c) 2012 TJ Holowaychuk <tj@vision-media.ca> (the original code) and Shelloid Systems LLP (only the Shelloid integration code).
 * MIT Licensed
 *
...
var pkg = lib_require("app_pkg");
var readCache = {};

var nonViewExports = ["clearCache", "configureExpress"];
var versions = {};
/**
 * Require cache.
 */
...
//Shelloid integration code:

for(var v in module.exports){
	if(nonViewExports.indexOf(v) < 0){
		(function(view){
			var viewFn = module.exports[view];		
			var renderFn = module.exports[view].render;			
			var wrapperGen = function(fn){
				var f = 
					function(){
						...
						if(requires[view]){
							...
							pkg.require(view, versions[view] || "*", 
								function(mod){
									requires[view] = mod;
									fn.call(null, args);
								}
			...
			module.exports[view] = wrapperGen(viewFn);
			module.exports[view].render = wrapperGen(renderFn);			
		})(v);
	}
}
